...
import { FileCode, Copy, Check, Settings, Search, Globe2, Clock, Shield, AlertCircle, TrendingUp, Database } from 'lucide-react'
...
export function WhoisRdapTool() {
    ...
    const [lookupHistory, setLookupHistory] = usePersistentState<Array<{query: string, result: string, timestamp: string}>>('whois_history', [])
    const [formatOutput, setFormatOutput] = usePersistentState('whois_format', 'pretty')
    ...
    const canLookup = useMemo(() => query.trim().length > 0, [query])
    ...
    const addToHistory = (queryValue: string, data: any) => {
        const newEntry = {
            query: queryValue,
    ...
    return (
        <ToolLayout
            title="WHOIS (RDAP)"
            description="Lookup domain or IP registration data via RDAP with advanced features."
            icon={Globe2}
                ...
                <div className="flex items-center justify-between p-4 glass rounded-2xl border">
                    <div className="flex items-center space-x-3">
                        ...
                        <div className="flex flex-col">
                            <h2 className="text-xl font-black text-[var(--text-primary)]">Advanced WHOIS (RDAP)</h2>
                            <p className="text-sm text-[var(--text-muted)]">Domain and IP registration data via RDAP protocol</p>
                        </div>
                    </div>
                ...
                {error && (
                    <div className="p-4 glass rounded-2xl border border-red-500/30 bg-red-500/5 text-red-400 text-xs font-mono">
                        <div className="flex items-center space-x-2 mb-1">
                            ...
                            <span className="font-bold">WHOIS Lookup Error</span>
                        </div>
                        {error}
                    </div>
                )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="glass rounded-2xl border p-5 bg-[var(--bg-secondary)]/30 lg:col-span-2">
                        <div className="flex items-center space-x-2 mb-3">
                            ...
                            <label className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest">Domain or IP</label>
                        </div>
                        <input 
                            type="text" 
                            value={query} 
                        ...
                        <div className="mt-3 flex items-center space-x-2">
                            ...
                            <p className="text-[10px] text-[var(--text-muted)] font-black uppercase tracking-widest">
                                {endpoint && (endpoint.includes('/domain/') ? 'Domain' : 'IP')} detected
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="p-4 glass rounded-2xl border">
                        ...
                        <div className="mt-4 p-3 glass rounded-lg border bg-[var(--bg-tertiary)]">
                            <div className="flex items-center space-x-2 mb-2">
                                ...
                                <span className="text-xs text-[var(--text-muted)] font-black uppercase tracking-widest">RDAP Service</span>
                            </div>
                            <p className="text-sm text-[var(--text-primary)]">
                                Uses rdap.org (free) for RDAP queries. May be blocked by CORS in some browsers.
                            </p>
                        </div>
                    </div>
                )}
                        ...
                        <div className="flex-1 glass rounded-2xl border bg-[#0d1117] shadow-inner relative overflow-hidden max-h-[600px]">
                            {result ? (
                                ...
                                <div className="p-8 text-center text-[var(--text-muted)] opacity-50">
                                    ...
                                    <p className="text-xs">Enter a domain name or IP address to query</p>
                                </div>
                            )}
                        </div>
                    </div>
                        ...
                        <div className="flex-1 glass rounded-2xl border bg-[#0d1117] shadow-inner relative overflow-hidden max-h-[600px]">
                            {lookupHistory.length > 0 ? (
                                ...
                                <div className="p-8 text-center text-[var(--text-muted)] opacity-50">
                                    ...
                                    <p className="text-xs">Your WHOIS lookup history will appear here</p>
                                </div>
                            )}
                        </div>
                    </div>
