...
import { Waves, Play, Sparkles, MoveRight } from 'lucide-react'
...
export function BezierTool() {
    ...
    const parseBezierValues = (val: string) => {
        const parts = val.replace('cubic-bezier(', '').replace(')', '').split(',').map(s => Number(s.trim()))
        if (parts.length === 4 && parts.every(p => !isNaN(p))) {
            setBezier(parts.join(', '))
        }
    }
        ...
        >
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 text-[var(--text-primary)]">
                <div className="space-y-8">
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center space-x-4 bg-[var(--input-bg)] p-6 rounded-[2rem] border border-[var(--border-primary)] group focus-within:ring-4 focus-within:ring-brand/10 transition-all shadow-inner">
                            ...
                            />
                            <button onClick={handlePreview} className="p-3 bg-brand/10 text-brand rounded-xl hover:scale-110 active:scale-95 transition-all">
                                <Play className="w-5 h-5 fill-current" />
                            </button>
                        </div>
                        <p className="text-[9px] text-[var(--text-muted)] font-black uppercase tracking-[0.1em] pl-4 opacity-50">
                            Coordinate Space: [x1, y1, x2, y2]
                        </p>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-3 px-2">
                            {PRESETS.map((p) => (
                                <button
                                    key={p.name}
                                    onClick={() => parseBezierValues(p.value)}
                                    className={cn(
                                        ...
                                        bezierValue === p.value ? "border-brand/40 bg-brand/5 ring-1 ring-brand/20 shadow-brand/5 scale-105" : ""
                                    )}
                                >
                                    <span className={cn(
                                        "text-[10px] font-black uppercase tracking-widest group-hover:text-brand transition-colors",
                                        bezierValue === p.value ? "text-brand" : "text-[var(--text-primary)] opacity-60"
                                    )}>{p.name}</span>
                                </button>
                            ))}
                        </div>
                ...
                <div className="space-y-8 flex flex-col">
                    <div className="p-10 glass rounded-[4rem] border-[var(--border-primary)] space-y-12 bg-[var(--bg-secondary)]/30 shadow-sm transition-all hover:bg-[var(--bg-secondary)]/40">
                        <div className="space-y-8">
                            ...
                            <div className="space-y-12">
                                <div className="space-y-4">
                                    <p className="text-[9px] text-[var(--text-muted)] uppercase font-black tracking-widest pl-2 opacity-60">X-Axis Linear Projection</p>
                                    <div className="h-24 glass rounded-[2.5rem] relative overflow-hidden border-[var(--border-primary)] bg-[var(--bg-primary)] shadow-inner">
                                        ...
                                        <div
                                            className={cn("w-16 h-16 bg-brand rounded-2xl absolute top-4 left-4 shadow-2xl shadow-brand/30 flex items-center justify-center text-white", isPlaying ? "translate-x-[calc(350%)]" : "translate-x-0")}
                                            style={{
                                                transition: `all 1.2s ${bezierValue}`,
                                            }}
                                        >
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="h-32 glass rounded-[2.5rem] flex items-center justify-center border-[var(--border-primary)] bg-[var(--bg-primary)] shadow-inner overflow-hidden">
                                        ...
                                        <div
                                            ...
                                            style={{
                                                transition: `all 0.8s ${bezierValue}`,
                            ...
                            <div className="flex justify-center pt-4">
                                <button
                                    ...
                                    className="px-12 py-4 bg-brand text-white font-black uppercase text-[10px] tracking-[0.2em] rounded-full hover:scale-105 active:scale-95 transition-all shadow-xl shadow-brand/20 border border-brand/20"
                                >
                                    Re-Initiate Kinetic Cycle
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="pt-10 border-t border-[var(--border-primary)] space-y-6">
                            <div className="flex items-center justify-between px-2">
                                ...
                                <button onClick={() => copyToClipboard(bezierValue)} className="text-[10px] text-brand font-black uppercase tracking-widest hover:underline decoration-2 underline-offset-4">Copy Primitive</button>
                            </div>
